/**
 * OAuth 2.1 Provider Implementation (Internal - Interface API Only)
 *
 * This provider is used internally by the Interface-driven API to support OAuth
 * configuration via IAuth interfaces. It implements the MCP SDK's OAuthServerProvider.
 *
 * NOT EXPORTED from public API - only used internally by authConfigFromParsed().
 *
 * For programmatic OAuth usage:
 * - Use external OAuth provider (Auth0, Okta, Keycloak)
 * - Implement SDK's OAuthServerProvider interface
 * - See examples/reference-oauth-provider.ts for reference implementation
 *
 * @internal
