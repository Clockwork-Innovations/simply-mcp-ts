...
export interface HandlerContext {
   ...
   * Only available when the client supports elicitation capability.
   * @param prompt The message to show the user
   * @param args The input fields to collect (JSON Schema format)
   * @returns The user's response with action (accept/decline/cancel) and collected input
   */
  elicitInput?: (prompt: string, args: Record<string, any>) => Promise<ElicitResult>;

  /**
   * Request the list of root directories from the client.
   * Only available when the client supports roots capability.
...
/**
 * Elicitation result from user input request
 * Based on MCP SDK ElicitResult type
 */
export interface ElicitResult {
  ...
  /**
   * The collected user input (only present if action is 'accept')
   * Keys match the field names from the args schema
   */
  content?: Record<string, any>;

  /**
   * Optional metadata from the elicitation
   */
