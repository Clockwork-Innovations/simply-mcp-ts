...
async function demo() {
  ...
  await execCommand('node', ['dist/mcp/cli/index.js', 'list']);
  ...
  const multiServer = spawn('node', [
    'dist/mcp/cli/index.js',
    'run',
    'mcp/examples/class-minimal.ts',
  ...
  await execCommand('node', ['dist/mcp/cli/index.js', 'list', '--verbose']);

  // Demo 4: List as JSON
  console.log(`\n${COLORS.yellow}Step 5: List as JSON${COLORS.reset}`);
  await execCommand('node', ['dist/mcp/cli/index.js', 'list', '--json']);

  // Demo 5: Stop all servers
  console.log(`\n${COLORS.yellow}Step 6: Stop all servers${COLORS.reset}`);
  process.env.SIMPLYMCP_AUTO_CONFIRM = 'true';
  await execCommand('node', ['dist/mcp/cli/index.js', 'stop', 'all']);
