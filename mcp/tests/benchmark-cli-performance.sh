...
# Test files
TEST_DECORATOR="mcp/examples/class-minimal.ts"
TEST_FUNCTIONAL="mcp/examples/function-server.ts"
TEST_PROGRAMMATIC="mcp/examples/simple-server.ts"
...
measure_performance() {
  ...
  local output=$(npx tsx mcp/cli/index.ts run "$file" --verbose --style="$style" --dry-run 2>&1 || true)
