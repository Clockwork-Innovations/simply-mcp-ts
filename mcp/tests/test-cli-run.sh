...
CLI_ROOT="$MCP_ROOT/dist/mcp/cli"
...
if grep -q "Detected API style: decorator" /tmp/cli-test-decorator.log 2>/dev/null; then
...
if grep -q "Loading class from" /tmp/cli-test-class-decorator.log 2>/dev/null && \
   grep -q "Creating server" /tmp/cli-test-class-decorator.log 2>/dev/null; then
...
if curl -s http://localhost:3333 > /dev/null 2>&1 || \
   curl -s http://localhost:3333/sse > /dev/null 2>&1; then
  ...
  if grep -qi "http" /tmp/cli-test-http.log 2>/dev/null || \
     grep -qi "port.*3333" /tmp/cli-test-http.log 2>/dev/null; then
    print_result "--http flag enables HTTP transport" "PASS"
  else
...
if npx tsx "$MCP_ROOT/cli/run-bin.ts" run "/tmp/nonexistent-file-12345.ts" > /tmp/cli-test-notfound.log 2>&1; then
  ...
  if grep -q "not found" /tmp/cli-test-notfound.log 2>/dev/null || \
     grep -q "ENOENT" /tmp/cli-test-notfound.log 2>/dev/null; then
...
if timeout 3 npx tsx "$MCP_ROOT/cli/class-bin.ts" "mcp/examples/single-file-basic.ts" > /tmp/cli-test-class-error.log 2>&1; then
  ...
  if grep -qi "error" /tmp/cli-test-class-error.log 2>/dev/null || \
     grep -qi "class" /tmp/cli-test-class-error.log 2>/dev/null; then
...
if timeout 3 npx tsx "$MCP_ROOT/cli/func-bin.ts" "mcp/examples/class-minimal.ts" > /tmp/cli-test-func-error.log 2>&1; then
  ...
  if grep -qi "error" /tmp/cli-test-func-error.log 2>/dev/null || \
     grep -qi "config" /tmp/cli-test-func-error.log 2>/dev/null; then
