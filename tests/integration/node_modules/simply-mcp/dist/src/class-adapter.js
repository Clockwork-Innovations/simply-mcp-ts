 ...
 */
export async function loadClass(classFile) {
    ...
    try {
        ...
        throw new Error(`Failed to load file: ${classFile}\n\n` +
            `Error: ${error.message}\n\n` +
            `Troubleshooting:\n` +
    ...
    const ServerClass = module.default || Object.values(module).find((exp) => typeof exp === 'function' && exp.prototype);
    if (!ServerClass) {
        throw new Error(`No MCP server class found in: ${classFile}\n\n` +
            `Expected:\n` +
 ...
 */
function mergeParameterTypes(runtimeParams, parsedParams) {
 ...
 */
export function createServerFromClass(ServerClass, sourceFilePath) {
    const config = getServerConfig(ServerClass);
    if (!config) {
        const className = ServerClass.name || 'UnnamedClass';
        throw new Error(`Class '${className}' must be decorated with @MCPServer\n\n` +
            `What went wrong:\n` +
...
async function main() {
    ...
    console.error(`[ClassAdapter] Loaded: ${stats.tools} tools, ${stats.prompts} prompts, ${stats.resources} resources`);
    try {
        await server.start({
...
//# sourceMappingURL=class-adapter.js.map