...
import { parseInlineDependencies } from './core/index.js';
...
import { createImageContent, createAudioContent, createBlobContent, bufferToBase64, isBuffer, isUint8Array, } from './core/content-helpers.js';
/**
...
export class SimplyMCP {
    ...
    registerToolHandlers() {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request, extra) => {
            ...
            try {
                ...
                const logNotificationCallback = this.options.capabilities?.logging && this.server
                    ? (level, message, data) => {
                ...
                if (this.options.capabilities?.sampling && this.server) {
                    context.sample = async (messages, options) => {
                ...
                if (progressToken !== undefined && this.server) {
                    context.reportProgress = async (progress, total, message) => {
    ...
    registerPromptHandlers() {
        ...
        this.server.setRequestHandler(GetPromptRequestSchema, async (request) => {
            ...
            const renderedText = this.renderTemplate(prompt.template, request.params.arguments || {});
            return {
                messages: [
                    {
                        role: 'user',
    ...
    async startStdio() {
        ...
        console.error(`[SimplyMCP] Starting '${this.options.name}' v${this.options.version} (stdio transport)`);
        console.error(`[SimplyMCP] Registered: ${this.tools.size} tools, ${this.prompts.size} prompts, ${this.resources.size} resources`);
    ...
    async startHttp(port, stateful) {
        ...
        if (isStateless) {
            ...
            app.post('/mcp', async (req, res) => {
                ...
                try {
                    // FIX #1: In stateful mode, reject non-initialize requests without session ID
                    if (!sessionId && !isInitializeRequest(req.body)) {
                        res.status(401).json({
                            jsonrpc: '2.0',
                            error: {
                                code: -32001,
                                message: 'Unauthorized: Mcp-Session-Id header required in stateful mode',
                            },
                            id: req.body?.id || null,
                        });
                        return;
            ...
            app.delete('/mcp', async (req, res) => {
                ...
                console.log(`[SimplyMCP] Terminating session ${sessionId}`);
                try {
                    const transport = this.transports.get(sessionId);
                    // FIX #2: Delete session IMMEDIATELY to prevent reuse
                    this.transports.delete(sessionId);
                    // Handle DELETE request (closes transport)
                    await transport.handleRequest(req, res);
                    // Ensure cleanup
                    try {
                        await transport.close();
                        ...
                        console.error('[SimplyMCP] Error closing transport:', closeError);
                    }
                }
                catch (error) {
        ...
        this.httpServer = app.listen(port, () => {
            console.log(`[SimplyMCP] Server '${this.options.name}' v${this.options.version} listening on port ${port}`);
            console.log(`[SimplyMCP] HTTP Mode: ${isStateful ? 'STATEFUL' : 'STATELESS'}`);
            console.log(`[SimplyMCP] Registered: ${this.tools.size} tools, ${this.prompts.size} prompts, ${this.resources.size} resources`);
        });
        // Handle graceful shutdown
        process.on('SIGINT', async () => {
            await this.stop();
     ...
     */
    async normalizeResult(result, logger) {
        ...
        if (isBuffer(result) || isUint8Array(result)) {
            try {
                const imageContent = await createImageContent(result, undefined, this.options.basePath, logger);
                return { content: [imageContent] };
            }
            catch (error) {
                throw new HandlerExecutionError(`Failed to convert binary result to image: ${error instanceof Error ? error.message : 'Unknown error'}`, 'BINARY_CONVERSION_ERROR', { originalError: error });
            }
        }
        ...
        if (result && typeof result === 'object') {
            if ('type' in result) {
                ...
                try {
                    ...
                    if (resultType === 'audio') {
                        const audioContent = await createAudioContent(result, undefined, this.options.basePath, logger);
                        return { content: [audioContent] };
                    }
                    if (resultType === 'binary') {
                        const binaryContent = await createBlobContent(result, undefined, this.options.basePath, logger);
                    ...
                    if (resultType === 'file') {
                        ...
                        if ('path' in result && typeof result.path === 'string') {
                            ...
                            const mimeType = 'mimeType' in result ? result.mimeType : undefined;
                            // Determine content type based on MIME type
                            if (mimeType?.startsWith('image/')) {
                                const imageContent = await createImageContent(result, mimeType, this.options.basePath, logger);
                                return { content: [imageContent] };
                            }
                            else if (mimeType?.startsWith('audio/')) {
                                const audioContent = await createAudioContent(result, mimeType, this.options.basePath, logger);
                                return { content: [audioContent] };
                            }
                            else {
                                // Default to binary content for other file types
                                const binaryContent = await createBlobContent(result, mimeType, this.options.basePath, logger);
                                return { content: [binaryContent] };
                            }
                        }
                    }
                    ...
                    throw new HandlerExecutionError(`Failed to convert ${resultType} content: ${error instanceof Error ? error.message : 'Unknown error'}`, 'CONTENT_CONVERSION_ERROR', { type: resultType, originalError: error });
                }
            }
        }
    ...
    static async fromFile(filePath, options) {
        ...
        if (parseResult.errors.length > 0) {
            const errorMessages = parseResult.errors
                ...
                .join('\n');
            throw new Error(`Failed to parse inline dependencies from ${filePath}:\n${errorMessages}`);
        }
     ...
     * @private
     */
    async requestSampling(messages, options) {
        if (!this.server) {
            throw new Error(`Server not initialized\n\n` +
                `What went wrong:\n` +
                `  Internal error: Server instance was not created properly.\n\n` +
        ...
        throw new Error(`LLM sampling not yet supported\n\n` +
            `What went wrong:\n` +
     ...
     * @private
     */
    sendLoggingNotification(level, message, data) {
        if (!this.server) {
            return;
        }
        const notification = {
...
//# sourceMappingURL=SimplyMCP.js.map