...
function convertSchema(schema, options) {
    ...
    if (Array.isArray(schema.type)) {
        const schemas = schema.type.map(type => convertSchema({ ...schema, type }, options));
        return z.union(schemas);
    }
 ...
 */
function applyStringFormat(zodSchema, format) {
    switch (format) {
        case 'email':
 ...
 */
function convertNumber(schema, isInteger) {
 ...
 */
function convertObject(schema, options) {
 ...
 */
function convertArray(schema, options) {
    if (!schema.items) {
    ...
    if (schema.uniqueItems === true) {
        return arraySchema.refine((arr) => new Set(arr.map((item) => JSON.stringify(item))).size === arr.length, {
