 ...
 */
export function createLLMFriendlyValidationError(toolName, toolDescription, schema, errors, receivedInput) {
 ...
 */
function extractExpectedFromMessage(message) {
 ...
 */
function generateFixInstruction(error, schema) {
 ...
 */
function generateSchemaHelp(schema) {
    const help = {};
    if (!schema.properties)
        return help;
 ...
 */
function generatePropertyExamples(schema) {
    const examples = [];
    ...
    switch (schema.type) {
        case 'string':
            if (schema.format === 'email')
                ...
                examples.push('2024-01-01');
            else if (schema.format === 'time')
                examples.push('12:30:00');
            else if (schema.pattern)
                examples.push('[match pattern]');
            else
 ...
 */
function generateSuggestions(errors, schema) {
    const suggestions = [];
    ...
    const rangeErrors = errors.filter(e => e.message.includes('minimum') ||
 ...
 */
export function formatErrorForLLM(response) {
...
//# sourceMappingURL=LLMFriendlyErrors.js.map