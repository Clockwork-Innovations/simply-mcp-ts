...
export class InputSanitizer {
    ...
    sanitizeString(input, field) {
        ...
        if (value.length > this.config.maxStringLength) {
            if (this.config.strictMode) {
                throw new SanitizationError(`String exceeds maximum length of ${this.config.maxStringLength}`, field, 'STRING_TOO_LONG');
            }
        ...
        if (this.hasDangerousPattern(value, InputSanitizer.SQL_PATTERNS)) {
            if (this.config.strictMode) {
                throw new SanitizationError('Potential SQL injection detected', field, 'SQL_INJECTION', value);
            }
        ...
        if (this.hasDangerousPattern(value, InputSanitizer.SHELL_PATTERNS)) {
            if (this.config.strictMode) {
                throw new SanitizationError('Potential shell injection detected', field, 'SHELL_INJECTION', value);
            }
        ...
        if (!this.config.allowHtml && this.hasDangerousPattern(value, InputSanitizer.XSS_PATTERNS)) {
            if (this.config.strictMode) {
                throw new SanitizationError('Potential XSS attack detected', field, 'XSS_ATTACK', value);
            }
        ...
        if (this.hasDangerousPattern(value, InputSanitizer.PATH_TRAVERSAL_PATTERNS)) {
            if (this.config.strictMode) {
                throw new SanitizationError('Path traversal detected', field, 'PATH_TRAVERSAL', value);
            }
        ...
        if (this.hasDangerousPattern(value, this.config.customDangerousPatterns)) {
            if (this.config.strictMode) {
                throw new SanitizationError('Custom dangerous pattern detected', field, 'CUSTOM_PATTERN', value);
            }
     ...
     */
    sanitizeObject(input, depth = 0, path = 'root') {
        const warnings = [];
        let modified = false;
        // Check depth limit
        if (depth > this.config.maxDepth) {
            throw new SanitizationError(`Object depth exceeds maximum of ${this.config.maxDepth}`, path, 'MAX_DEPTH_EXCEEDED');
        }
        ...
        if (Array.isArray(input)) {
            if (input.length > this.config.maxArrayLength) {
                if (this.config.strictMode) {
                    throw new SanitizationError(`Array exceeds maximum length of ${this.config.maxArrayLength}`, path, 'ARRAY_TOO_LONG');
                }
        ...
        if (typeof input === 'object') {
            ...
            for (const [key, value] of Object.entries(input)) {
                ...
                if (keyResult.modified)
                    modified = true;
                warnings.push(...keyResult.warnings);
                // Sanitize the value
                const valueResult = this.sanitizeObject(value, depth + 1, `${path}.${keyResult.value}`);
     ...
     */
    hasDangerousPattern(input, patterns) {
...
export function sanitizeObject(input, depth) {
    return defaultSanitizer.sanitizeObject(input, depth);
}
export function sanitizeForSql(input, field) {
    return defaultSanitizer.sanitizeForSql(input, field);
}
export function sanitizeForShell(input, field) {
    return defaultSanitizer.sanitizeForShell(input, field);
}
export function sanitizeHtml(input, field) {
