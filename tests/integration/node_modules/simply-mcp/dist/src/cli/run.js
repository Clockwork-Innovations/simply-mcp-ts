...
import { loadCLIConfig, mergeRunConfig, mergeServerConfig, getConfigFilePath, resolveServerConfig, listServers, } from './cli-config-loader.js';
/**
 ...
 */
async function loadTypeScriptFile(absolutePath) {
 ...
 */
export async function detectAPIStyle(filePath) {
    try {
 ...
 */
async function runFunctionalAdapter(filePath, useHttp, useHttpStateless, port, verbose = false) {
    ...
    if (config.tools && config.tools.length > 0) {
        for (const tool of config.tools) {
            ...
            const parameters = isZodSchema ? tool.parameters : schemaToZod(tool.parameters);
            server.addTool({
                name: tool.name,
 ...
 */
async function runDecoratorAdapter(filePath, useHttp, useHttpStateless, port, verbose = false) {
    ...
    const { getServerConfig, getTools, getPrompts, getResources, getParameterNames, getParameterInfo, inferZodSchema, extractJSDoc, } = await import(pathToFileURL(resolve(distPath, 'decorators.js')).href);
    const { parseTypeScriptFileWithCache, getMethodParameterTypes } = await import(pathToFileURL(resolve(distPath, 'type-parser.js')).href);
    const { startServer, displayServerInfo } = await import('./adapter-utils.js');
    // Load the class
    const absolutePath = resolve(process.cwd(), filePath);
    const module = await loadTypeScriptFile(absolutePath);
    ...
    const getPublicMethods = (instance) => {
        const methods = [];
    ...
    const mergeParameterTypes = (runtimeParams, parsedParams) => {
 ...
 */
async function runInterfaceAdapter(filePath, useHttp, useHttpStateless, port, verbose = false) {
...
/**
 * Check if a value is a SimplyMCP server instance
 */
function isSimplyMCPInstance(value) {
    if (!value || typeof value !== 'object') {
        return false;
    }
    // Check for required SimplyMCP instance methods
    return (typeof value.addTool === 'function' &&
 ...
 */
async function runProgrammaticAdapter(filePath, useHttp, useHttpStateless, port, verbose = false) {
    const absolutePath = resolve(process.cwd(), filePath);
    try {
        // Import the module
        const module = await loadTypeScriptFile(absolutePath);
        // Check if module exports a SimplyMCP instance
        ...
        if (serverInstance && isSimplyMCPInstance(serverInstance)) {
            ...
            if (verbose) {
                console.error('[RunCommand] Detected exported SimplyMCP instance');
            ...
            console.error('[RunCommand] Note: Programmatic servers manage their own transport configuration');
        }
    }
    catch (error) {
 ...
 */
async function runMCPBuilderAdapter(filePath, useHttp, useHttpStateless, port, verbose = false) {
 ...
 */
async function discoverServers(cwd = process.cwd()) {
    try {
 ...
 */
async function parseConfigAsText(configPath) {
    try {
 ...
 */
async function showServerDiscovery(config, configFilePath) {
 ...
 */
async function spawnWithInspector(inspectFlag, inspectPort, argv) {
    ...
    if (argv.config)
        scriptArgs.push('--config', argv.config);
    if (argv.http)
        scriptArgs.push('--http');
    if (argv['http-stateless'])
        scriptArgs.push('--http-stateless');
    if (argv.port)
        scriptArgs.push('--port', String(argv.port));
    if (argv.style)
        scriptArgs.push('--style', argv.style);
    if (argv.verbose)
    ...
    if (argv['watch-poll'])
        scriptArgs.push('--watch-poll');
    if (argv['watch-interval'])
        scriptArgs.push('--watch-interval', String(argv['watch-interval']));
...
export const runCommand = {
    ...
    builder: (yargs) => {
        return yargs
            ...
            choices: ['interface', 'decorator', 'functional', 'programmatic', 'mcp-builder'],
            type: 'string',
        })
            .option('verbose', {
    ...
    handler: async (argv) => {
        ...
        try {
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                if (serverConfig) {
                    // Named server found
                    const merged = mergeServerConfig(serverConfig.config, config?.defaults || null, cliOptions);
            ...
            let useHttpStateless = argv['http-stateless'];
            ...
            const inspectBrk = argv['inspect-brk'];
            const inspectPort = argv['inspect-port'];
