 ...
 */
function findTypeScriptFiles(dir, recursive) {
    const results = [];
    try {
 ...
 */
function startServerProcess(options, state) {
    if (state.child) {
 ...
 */
function stopServerProcess(state, verbose) {
 ...
 * Restart the server
 */
async function restartServer(options, state) {
    const now = Date.now();
 ...
 * Start watch mode
 */
export async function startWatchMode(options) {
