...
import { loadCLIConfig, getConfigFilePath, validateConfig, listServers, } from './cli-config-loader.js';
/**
 ...
 */
async function listAvailableServers(configPath) {
    try {
        const configFile = await getConfigFilePath(configPath);
        if (!configFile) {
            console.error('No configuration file found');
 ...
 */
async function initConfig(format = 'ts') {
    try {
        const fileName = format === 'ts'
            ? 'simplymcp.config.ts'
            : format === 'js'
                ? 'simplymcp.config.js'
...
export const configCommand = {
    ...
    builder: (yargs) => {
        return yargs
            ...
            choices: ['show', 'validate', 'list', 'init'],
            demandOption: true,
        })
            .option('config', {
            alias: 'c',
