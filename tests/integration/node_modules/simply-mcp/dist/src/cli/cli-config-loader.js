 ...
 */
export async function loadCLIConfig(configPath, cwd = process.cwd()) {
 ...
 */
async function findCLIConfigFile(cwd) {
    const candidates = [
        'simplymcp.config.ts',
        'simplymcp.config.js',
 ...
 */
async function parseCLIConfig(configPath) {
 ...
 */
function validateCLIConfig(config, configPath) {
        ...
        for (const [serverName, serverConfig] of Object.entries(config.servers)) {
            ...
            if (sc.style !== undefined) {
                ...
                if (!validStyles.includes(sc.style)) {
                    throw new Error(`Server config "${serverName}".style must be one of: ${validStyles.join(', ')} in: ${configPath}`);
                }
            }
            // Validate transport
            if (sc.transport !== undefined) {
                ...
                if (!validTransports.includes(sc.transport)) {
                    throw new Error(`Server config "${serverName}".transport must be one of: ${validTransports.join(', ')} in: ${configPath}`);
                }
            }
            // Validate port
            if (sc.port !== undefined) {
                if (typeof sc.port !== 'number' || sc.port < 1 || sc.port > 65535) {
                    throw new Error(`Server config "${serverName}".port must be a number between 1 and 65535 in: ${configPath}`);
                }
            }
            // Validate watch
            ...
            if (sc.env !== undefined) {
                ...
                for (const [key, value] of Object.entries(sc.env)) {
                    if (typeof value !== 'string') {
                        throw new Error(`Server config "${serverName}".env["${key}"] must be a string in: ${configPath}`);
                    }
                }
            }
            // Validate verbose
    ...
    if (config.defaults !== undefined) {
        ...
        if (defaults.transport !== undefined) {
            ...
            if (!validTransports.includes(defaults.transport)) {
                throw new Error(`Config field "defaults.transport" must be one of: ${validTransports.join(', ')} in: ${configPath}`);
            }
        }
        // Validate port
        if (defaults.port !== undefined) {
            if (typeof defaults.port !== 'number' || defaults.port < 1 || defaults.port > 65535) {
                throw new Error(`Config field "defaults.port" must be a number between 1 and 65535 in: ${configPath}`);
            }
        }
        // Validate verbose
    ...
    if (config.run !== undefined) {
        ...
        if (config.run.style !== undefined) {
            ...
            if (!validStyles.includes(config.run.style)) {
                throw new Error(`Config field "run.style" must be one of: ${validStyles.join(', ')} in: ${configPath}`);
            }
        }
        // Validate run.http
    ...
    if (config.bundle !== undefined) {
        ...
        if (config.bundle.format !== undefined) {
            ...
            if (!validFormats.includes(config.bundle.format)) {
                throw new Error(`Config field "bundle.format" must be one of: ${validFormats.join(', ')} in: ${configPath}`);
            }
        }
        ...
        if (config.bundle.platform !== undefined) {
            ...
            if (!validPlatforms.includes(config.bundle.platform)) {
                throw new Error(`Config field "bundle.platform" must be one of: ${validPlatforms.join(', ')} in: ${configPath}`);
            }
        }
        ...
        if (config.bundle.target !== undefined) {
            ...
            if (!validTargets.includes(config.bundle.target)) {
                throw new Error(`Config field "bundle.target" must be one of: ${validTargets.join(', ')} in: ${configPath}`);
            }
        }
        ...
        if (config.bundle.external !== undefined) {
            ...
            if (!config.bundle.external.every((item) => typeof item === 'string')) {
    ...
    if (config.output !== undefined) {
        ...
        if (config.output.format !== undefined) {
            ...
            if (!validFormats.includes(config.output.format)) {
                throw new Error(`Config field "output.format" must be one of: ${validFormats.join(', ')} in: ${configPath}`);
            }
        }
    }
}
 ...
 */
export function mergeRunConfig(config, cliOptions) {
    const merged = {};
 ...
 * @returns Path to config file or null if not found
 */
export async function getConfigFilePath(configPath, cwd = process.cwd()) {
    if (configPath) {
 ...
 */
export function resolveServerConfig(file, config) {
    if (!config?.servers) {
        return null;
    }
 ...
 */
export function mergeServerConfig(serverConfig, defaults, cliOptions) {
    const merged = {};
    // Helper to get value with priority
    const getValue = (cliKey, serverKey, defaultKey) => {
 ...
 */
export async function validateServerEntry(serverName, serverConfig, cwd) {
 ...
 */
export async function validateConfig(config, cwd = process.cwd()) {
    const errors = [];
    const warnings = [];
    ...
    if (config.servers) {
        for (const [serverName, serverConfig] of Object.entries(config.servers)) {
            ...
            if (serverConfig.transport === 'http' && !serverConfig.port && !config.defaults?.port) {
                warnings.push(`Server "${serverName}": HTTP transport configured without explicit port (will use default 3000)`);
            }
        }
    }
    return {
...
//# sourceMappingURL=cli-config-loader.js.map