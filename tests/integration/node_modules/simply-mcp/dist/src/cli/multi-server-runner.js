...
import { registerServer, unregisterServer, generateGroupId, findAvailablePort, } from './server-tracker.js';
 ...
 */
function prefixMessage(serverName, port, message, color) {
...
async function startServerProcess(options) {
    ...
    serverProcess.on('exit', (code, signal) => {
        if (code !== 0 && code !== null) {
            console.error(prefixMessage(serverName, port, `Server exited with code ${code}`, COLORS.red));
        }
        if (signal) {
            console.error(prefixMessage(serverName, port, `Server killed by signal ${signal}`, COLORS.red));
        }
    ...
    serverProcess.on('error', (error) => {
        console.error(prefixMessage(serverName, port, `Error: ${error.message}`, COLORS.red));
    });
 ...
 */
export async function runMultipleServers(options) {
...
//# sourceMappingURL=multi-server-runner.js.map