 ...
 */
function validateServerConfig(name, version, errors, warnings) {
    if (!name) {
 ...
 */
function validateToolNames(tools, errors, warnings) {
    const seen = new Set();
 ...
 */
async function dryRunDecorator(filePath, useHttp, port) {
    const errors = [];
    const warnings = [];
    const tools = [];
    const prompts = [];
    ...
    try {
        ...
        const { getServerConfig, getTools, getPrompts, getResources, extractJSDoc, } = await import(pathToFileURL(resolve(distPath, 'decorators.js')).href);
        const { parseTypeScriptFileWithCache } = await import(pathToFileURL(resolve(distPath, 'type-parser.js')).href);
        // Load the class
 ...
 */
async function dryRunFunctional(filePath, useHttp, port) {
    const errors = [];
    const warnings = [];
    const tools = [];
    const prompts = [];
 ...
 */
async function dryRunInterface(filePath, useHttp, port) {
    const errors = [];
    const warnings = [];
    const tools = [];
    const prompts = [];
    ...
    try {
        ...
        const __dirname = dirname(fileURLToPath(import.meta.url));
        const distPath = resolve(__dirname, '..');
        const { loadInterfaceServer } = await import(pathToFileURL(resolve(distPath, 'api/interface/adapter.js')).href);
 ...
 */
async function dryRunProgrammatic(filePath, useHttp, port) {
    const errors = [];
    const warnings = [];
 ...
 */
export async function runDryRun(filePath, style, useHttp, port, jsonOutput = false) {
    let result;
...
export async function dryRunDecoratorAdapter(filePath, useHttp, port) {
    await runDryRun(filePath, 'decorator', useHttp, port, false);
}
export async function dryRunFunctionalAdapter(filePath, useHttp, port) {
    await runDryRun(filePath, 'functional', useHttp, port, false);
}
export async function dryRunProgrammaticAdapter(filePath, useHttp, port) {
