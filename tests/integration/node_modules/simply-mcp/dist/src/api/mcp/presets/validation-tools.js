 ...
 */
export const ValidationToolsPreset = {
    ...
    tools: [
        {
            ...
            execute: async (args, context) => {
                ...
                if (args.tool_name.includes(' '))
                    basicIssues.push('Tool name should use snake_case (no spaces)');
                if (args.tool_name.includes('-'))
                    basicIssues.push('Tool name should use snake_case (use _ not -)');
                if (args.description.length < 20)
                    basicIssues.push('Description too brief (minimum 20 characters)');
                if (args.parameters.length === 0)
                    basicIssues.push('Tool should have at least one parameter');
...
Be specific and actionable in your feedback.`
        ...
        },
        {
            ...
            execute: async (args, context) => {
                // Basic syntax check
                const basicIssues = [];
                if (!args.schema_code.includes('z.object'))
                    basicIssues.push('Schema should use z.object()');
                if (!args.schema_code.includes('.describe('))
                    basicIssues.push('Schema should have .describe() on fields');
                if (!context?.sample) {
                    return {
                        content: [{
                                type: 'text',
        ...
        },
        {
            ...
            execute: async (args, context) => {
                ...
                if (happyPaths.length === 0)
                    basicIssues.push('No happy path tests provided');
                if (edgeCases.length === 0)
                    basicIssues.push('No edge case tests provided');
                if (errorCases.length === 0)
                    basicIssues.push('No error case tests provided');
                if (!context?.sample) {
                    return {
                        content: [{
                                type: 'text',
