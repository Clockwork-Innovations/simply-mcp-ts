 ...
 * ```
 */
export function createServerFromMCPBuilder(config) {
    // Validate config
    if (!config.name || !config.version) {
        throw new Error('MCP Builder config must have "name" and "version" properties.\n\n' +
            'Example:\n' +
    ...
    if (config.toolPresets && config.toolPresets.length > 0) {
        console.error(`[MCP Builder] Loading ${config.toolPresets.length} tool preset(s)...`);
    ...
    if (config.promptPresets && config.promptPresets.length > 0) {
        console.error(`[MCP Builder] Loading ${config.promptPresets.length} prompt preset(s)...`);
    ...
    if (config.resourcePresets && config.resourcePresets.length > 0) {
        console.error(`[MCP Builder] Loading ${config.resourcePresets.length} resource preset(s)...`);
    ...
    console.error(`[MCP Builder] Server "${config.name}" ready with ${totalTools} tools, ${totalPrompts} prompts, ${totalResources} resources`);
    return server;
}
/**
 ...
 * ```
 */
export async function loadMCPBuilderServer(filePath) {
    ...
    if (!config) {
        throw new Error(`MCP Builder file must have a default export.\n\n` +
            `File: ${filePath}\n\n` +
            `Example:\n` +
 ...
 * ```
 */
export async function isMCPBuilderFile(filePath) {
    try {
