 ...
 */
export function extractValidationTags(node, sourceFile) {
    const tags = {};
 ...
 */
export function typeNodeToZodSchema(typeNode, sourceFile, validationTags) {
    ...
    if (ts.isUnionTypeNode(typeNode)) {
        ...
        const hasUndefined = typeNode.types.some(t => t.kind === ts.SyntaxKind.UndefinedKeyword);
        if (hasUndefined && typeNode.types.length === 2) {
            // It's an optional type - process the non-undefined type
            const nonUndefinedType = typeNode.types.find(t => t.kind !== ts.SyntaxKind.UndefinedKeyword);
            if (nonUndefinedType) {
 ...
 */
function typeLiteralToZodSchema(typeLiteral, sourceFile) {
    const shape = {};
 ...
 */
export function generateSchemaFromTypeString(typeString, paramsNode, sourceFile) {
