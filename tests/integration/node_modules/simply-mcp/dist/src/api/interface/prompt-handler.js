 ...
 */
function parseArgsType(argsType) {
    const args = [];
...
export function interpolateTemplate(template, args) {
    ...
    result = result.replace(/\{(\w+)\s*\?\s*'([^']+)'\s*:\s*'([^']+)'\}/g, (_, variable, trueValue, falseValue) => {
 ...
 */
export function registerStaticPrompt(server, prompt) {
    const { name, description, template } = prompt;
    if (!template) {
        throw new Error(`Static prompt "${name}" is missing template string. ` +
 ...
 */
export function registerDynamicPrompt(server, serverInstance, prompt) {
    const { name, description, methodName } = prompt;
    // Check if method exists on server instance
    const method = serverInstance[methodName];
    if (!method) {
        throw new Error(`Dynamic prompt "${name}" requires method "${methodName}" but it was not found on server class.\n` +
            `Expected: class implements { ${methodName}: ${prompt.interfaceName} }`);
    }
    if (typeof method !== 'function') {
        throw new Error(`Dynamic prompt "${name}" method "${methodName}" is not a function (found: ${typeof method})`);
    }
 ...
 */
export function registerPrompts(server, serverInstance, prompts, verbose) {
