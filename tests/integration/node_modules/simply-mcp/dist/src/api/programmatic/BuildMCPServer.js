...
export class BuildMCPServer {
    ...
    async startStdio() {
        ...
        console.error(`[BuildMCPServer] Starting '${this.options.name}' v${this.options.version} (stdio transport)`);
        console.error(`[BuildMCPServer] Registered: ${this.tools.size} tools, ${this.prompts.size} prompts, ${this.resources.size} resources`);
    ...
    async startHttp(port, stateful) {
        ...
        this.httpServer = app.listen(port, () => {
            console.log(`[BuildMCPServer] Server '${this.options.name}' v${this.options.version} listening on port ${port}`);
            console.log(`[BuildMCPServer] HTTP Mode: ${isStateful ? 'STATEFUL' : 'STATELESS'}`);
            console.log(`[BuildMCPServer] Registered: ${this.tools.size} tools, ${this.prompts.size} prompts, ${this.resources.size} resources`);
        });
        // Handle graceful shutdown
        process.on('SIGINT', async () => {
            await this.stop();
    ...
    async requestSampling(messages, options) {
        ...
        try {
            ...
            const result = await this.server.createMessage({
                messages: messages, // Cast needed - SamplingMessage format matches CreateMessageRequest
     ...
     */
    async getPromptDirect(promptName, args = {}) {
        const prompt = this.prompts.get(promptName);
        if (!prompt) {
            const availablePrompts = Array.from(this.prompts.keys()).join(', ') || 'none';
            throw new Error(`Unknown prompt: ${promptName}\n\n` +
