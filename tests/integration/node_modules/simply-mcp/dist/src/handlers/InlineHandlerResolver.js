...
import { HandlerSyntaxError, HandlerTimeoutError, HandlerConfigError, } from '../core/errors.js';
/**
 * Resolver for inline code handlers
 * Executes JavaScript code strings in a sandboxed VM context
 */
export class InlineHandlerResolver {
    defaultTimeout;
    /**
    ...
    async resolve(config) {
        if (!this.canResolve(config)) {
            throw new HandlerConfigError(`InlineHandlerResolver cannot resolve handler of type: ${config.type}`);
        }
        ...
        try {
            ...
            const isFunctionExpression = trimmedCode.startsWith('async ') ||
            ...
            throw new HandlerSyntaxError(`Syntax error in inline handler code: ${error instanceof Error ? error.message : String(error)}`, { code: inlineConfig.code, error: String(error) });
        }
        ...
        return async (args, context) => {
            ...
            try {
                ...
                throw new HandlerSyntaxError(`Runtime error in inline handler: ${error instanceof Error ? error.message : String(error)}`, { code: inlineConfig.code, error: String(error) });
            }
        };
    }
    /**
    ...
    createTimeoutPromise(timeout) {
        return new Promise((_, reject) => {
            setTimeout(() => {
                reject(new HandlerTimeoutError(`Handler execution exceeded timeout of ${timeout}ms`, {
                    timeout,
                }));
            }, timeout);
        });
...
//# sourceMappingURL=InlineHandlerResolver.js.map