...
export class RateLimiter {
     ...
     */
    recordRequest(sessionId, toolName) {
        if (!this.config.enabled) {
            return;
        }
        const limit = this.getLimit(toolName);
     ...
     */
    checkFixedWindow(key, window, maxRequests) {
        const now = Date.now();
     ...
     */
    checkSlidingWindow(key, window, maxRequests) {
        const now = Date.now();
        let bucket = this.buckets.get(key);
        if (!bucket) {
            bucket = {
