 ...
 */
export function enforcePermissions(...requiredPermissions) {
    return (req, res, next) => {
        ...
        const permissions = req.permissionChecker;
        if (!permissions) {
            res.status(500).json({
                jsonrpc: '2.0',
                error: {
 ...
 */
export function enforceRateLimit(rateLimiter, auditLogger, toolName) {
    return (req, res, next) => {
        const sessionId = req.headers['mcp-session-id'];
        const securityContext = req.securityContext;
        if (!sessionId) {
        ...
        if (!result.allowed) {
            ...
            if (securityContext) {
                auditLogger.logRateLimitExceeded(securityContext, toolName, result.currentRequests, result.maxRequests);
            }
        ...
        if (result.currentRequests >= result.maxRequests * 0.8 && securityContext) {
            auditLogger.logRateLimitWarning(securityContext, toolName, result.currentRequests, result.maxRequests);
        }
        next();
    };
}
 ...
 */
export function checkPermission(permissions, auditLogger, context, requiredPermission, resource) {
    const granted = permissions.hasPermission(context, requiredPermission);
    // Log the permission check
    auditLogger.logPermissionCheck(granted, context, [requiredPermission], resource);
    return granted;
}
/**
 * Helper to create permission denied error
 */
export function createPermissionDeniedError(requiredPermissions) {
    return {
        jsonrpc: '2.0',
        error: {
            code: -32002,
