 ...
 * ```
 */
export function parseInlineDependencies(sourceCode, options = {}) {
    const { strict = false, validateSemver = true, allowComments = true, } = options;
    const errors = [];
    const warnings = [];
    const dependencies = {};
    ...
    if (strict && errors.length > 0) {
        ...
        throw new Error(`Failed to parse inline dependencies:\n${errorMessages}`);
    }
    return {
        dependencies,
        errors,
 ...
 */
export function extractDependencyBlock(sourceCode) {
 ...
 */
export function parseDependencyLine(line, lineNumber) {
 ...
 */
export function parseInlineDependenciesDetailed(sourceCode, options = {}) {
    const result = parseInlineDependencies(sourceCode, options);
    // Convert map to array of Dependency objects
    const dependencies = Object.entries(result.dependencies).map(([name, version]) => ({
        name,
        version,
    }));
    return {
