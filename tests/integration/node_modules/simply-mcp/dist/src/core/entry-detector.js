 ...
 * ```
 */
export async function detectEntryPoint(providedEntry, basePath = process.cwd()) {
    ...
    throw new Error('No SimplyMCP entry point found. Please provide an entry point or create one of: ' +
 ...
 */
export function resolveEntryPath(entryPath, basePath) {
 ...
 */
export async function validateSimplyMCPEntry(filePath) {
    // Check file exists
    try {
    ...
    try {
        ...
        throw new Error(`Failed to read entry point: ${filePath}. ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
    ...
    const hasImport = /from\s+['"]simply-mcp['"]/.test(content) ||
    ...
    if (!hasImport) {
        throw new Error(`Entry point does not appear to import SimplyMCP: ${filePath}\n` +
    ...
    const hasInstantiation = /new\s+SimplyMCP\s*\(/.test(content) ||
    ...
    if (!hasInstantiation) {
        throw new Error(`Entry point does not appear to create a SimplyMCP instance: ${filePath}\n` +
 ...
 */
export async function isSimplyMCPFile(filePath) {
    try {
 ...
 * @returns Server name
 */
export async function extractServerName(filePath) {
    try {
 ...
 */
export function isTypeScriptEntry(filePath) {
 ...
 */
export async function isESMEntry(filePath, basePath) {
    ...
    try {
        const content = await readFile(filePath, 'utf-8');
        const hasESMSyntax = /^\s*import\s+/m.test(content) ||
