 ...
 * ```
 */
export function generatePackageJson(deps, options = {}) {
    const { name = 'mcp-server', version = '1.0.0', devDeps = [], peerDeps = [] } = options;
    // Convert array to map if needed
    const depsMap = Array.isArray(deps)
        ? deps.reduce((acc, dep) => {
 ...
 * ```
 */
export function mergeDependencies(inlineDeps, packageJson) {
    const merged = {};
    const conflicts = [];
    const warnings = [];
    ...
    for (const [name, version] of Object.entries(inlineDeps)) {
        if (pkgDeps[name]) {
            if (pkgDeps[name] !== version) {
                conflicts.push(name);
                warnings.push(`Conflict for "${name}": inline deps specify "${version}" but package.json has "${pkgDeps[name]}" (using package.json version)`);
            }
        }
        else {
 ...
 * ```
 */
export function formatDependencyList(deps, options = {}) {
 ...
 */
export function dependencyArrayToMap(deps) {
 ...
 */
export function dependencyMapToArray(deps) {
 ...
 */
export function sortDependencies(deps) {
    const sorted = {};
 ...
 * ```
 */
export function filterDependencies(deps, pattern) {
 ...
 */
export function getDependencyStats(deps) {
