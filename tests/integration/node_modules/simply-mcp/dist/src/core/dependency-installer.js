...
export async function installDependencies(dependencies, options = {}) {
    ...
    const { packageManager: pmOption = 'auto', cwd = process.cwd(), timeout = 5 * 60 * 1000, // 5 minutes default
    retries = 3, ignoreScripts = true, production = false, force = false, onProgress, onError, } = options;
    const installed = [];
    const failed = [];
    const skipped = [];
    const errors = [];
 ...
 */
function buildInstallArgs(packageManager, packages, options) {
 ...
 */
async function executeInstall(packageManager, args, options) {
 ...
 */
function parseInstallOutput(line, packageManager) {
