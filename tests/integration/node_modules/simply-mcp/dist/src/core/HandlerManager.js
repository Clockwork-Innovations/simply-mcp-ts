 ...
 */
import { HandlerExecutionError, HandlerConfigError, HandlerTimeoutError, } from './errors.js';
...
export class HandlerManager {
    ...
    async resolveHandler(config) {
        ...
        if (!resolver) {
            throw new HandlerConfigError(`No resolver found for handler type: ${config.type}`, { type: config.type });
        }
        try {
     ...
     */
    async executeHandler(handler, args, context, options = {}) {
        ...
        try {
            ...
            const result = await this.executeWithTimeout(handler, args, context, timeout, options.abortSignal);
     ...
     */
    async executeWithTimeout(handler, args, context, timeout, abortSignal) {
    ...
    wrapError(error, context) {
        ...
        return new HandlerExecutionError(`Handler execution failed: ${error instanceof Error ? error.message : String(error)}`, 'HANDLER_EXECUTION_ERROR', {
            sessionId: context.sessionId,
