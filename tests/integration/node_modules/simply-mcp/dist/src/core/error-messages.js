...
export const ErrorMessages = {
     ...
     */
    INVALID_SERVER_CLASS: (file) => `No MCP server class found in: ${file}\n\n` +
        `Expected:\n` +
     ...
     */
    FILE_LOAD_ERROR: (file, error, resolvedPath) => `Failed to load file: ${file}\n\n` +
        `Error: ${error}\n\n` +
        `Troubleshooting:\n` +
     ...
     */
    MISSING_SERVER_DECORATOR: (className) => `Class '${className}' must be decorated with @MCPServer\n\n` +
        `What went wrong:\n` +
     ...
     */
    INVALID_TOOL_CONFIG: (toolName, issue) => `Invalid tool configuration for '${toolName}'\n\n` +
        `Issue: ${issue}\n\n` +
        `Required fields:\n` +
     ...
     */
    DUPLICATE_TOOL: (toolName) => `Tool '${toolName}' is already registered\n\n` +
        `What went wrong:\n` +
     ...
     */
    INVALID_PROMPT_CONFIG: (promptName, issue) => `Invalid prompt configuration for '${promptName}'\n\n` +
        `Issue: ${issue}\n\n` +
        `Required fields:\n` +
     ...
     */
    DUPLICATE_PROMPT: (promptName) => `Prompt '${promptName}' is already registered\n\n` +
        `What went wrong:\n` +
     ...
     */
    INVALID_RESOURCE_CONFIG: (resourceUri, issue) => `Invalid resource configuration for '${resourceUri}'\n\n` +
        `Issue: ${issue}\n\n` +
        `Required fields:\n` +
     ...
     */
    DUPLICATE_RESOURCE: (resourceUri) => `Resource with URI '${resourceUri}' is already registered\n\n` +
        `What went wrong:\n` +
     ...
     */
    SERVER_ALREADY_RUNNING: () => `Cannot modify server: Server is already running\n\n` +
        `What went wrong:\n` +
     ...
     */
    SERVER_INIT_ERROR: (error) => `Failed to initialize MCP server\n\n` +
        `Error: ${error}\n\n` +
        `Troubleshooting:\n` +
     ...
     */
    TRANSPORT_ERROR: (transport, error) => `Failed to start ${transport} transport\n\n` +
        `Error: ${error}\n\n` +
        `Troubleshooting:\n` +
        `  For HTTP transport:\n` +
     ...
     */
    CANNOT_ADD_AFTER_START: (itemType) => `Cannot add ${itemType}s after server has started\n\n` +
        `What went wrong:\n` +
 ...
 */
export function formatError(message, context) {
 ...
 */
export function missingFieldError(field, location) {
 ...
 * @returns Error message
 */
export function typeMismatchError(field, expected, received) {
    return (`Type error for field '${field}'\n\n` +
        `Expected: ${expected}\n` +
        `Received: ${received}\n\n` +
