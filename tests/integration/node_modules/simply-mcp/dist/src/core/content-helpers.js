 ...
 */
export function detectMimeTypeFromExtension(filePath) {
 ...
 */
export function detectMimeType(input, filePath, providedMimeType) {
 ...
 */
export function sanitizeFilePath(filePath, basePath = process.cwd()) {
 ...
 */
export async function readBinaryFile(filePath, basePath = process.cwd()) {
    // Sanitize path
 ...
 */
export async function createImageContent(input, mimeType, basePath = process.cwd(), logger) {
    let buffer;
    ...
    if (buffer.length > MAX_SAFE_SIZE && logger) {
        logger.warn(`Large image file detected: ${buffer.length} bytes. ` +
 ...
 */
export async function createAudioContent(input, mimeType, basePath = process.cwd(), logger) {
    let buffer;
    let detectedMimeType;
    let originalPath;
    // Handle different input types
    ...
    if (buffer.length > MAX_SAFE_SIZE && logger) {
        logger.warn(`Large audio file detected: ${buffer.length} bytes. ` +
 ...
 */
export async function createBlobContent(input, mimeType, basePath = process.cwd(), logger) {
    let buffer;
    let detectedMimeType;
    let originalPath;
    // Handle different input types
    ...
    if (buffer.length > MAX_SAFE_SIZE && logger) {
        logger.warn(`Large binary file detected: ${buffer.length} bytes. ` +
