 ...
 * ```
 */
export async function checkDependencies(required, cwd) {
 ...
 * ```
 */
export async function isPackageInstalled(packageName, cwd) {
    const workingDir = cwd || process.cwd();
    try {
 ...
 * ```
 */
export async function getInstalledVersion(packageName, cwd) {
    const workingDir = cwd || process.cwd();
    try {
        const packageJsonPath = join(workingDir, 'node_modules', packageName, 'package.json');
 ...
 * ```
 */
export function verifyVersion(installed, required) {
 ...
 */
function compareVersions(major1, minor1, patch1, major2, minor2, patch2) {
    if (major1 !== major2)
