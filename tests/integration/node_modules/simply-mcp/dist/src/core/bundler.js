 ...
 */
function buildEsbuildConfig(options, entry, deps) {
    ...
    if (format === 'single-file') {
        ...
        if (deps.nativeModules && deps.nativeModules.length > 0) {
            throw new Error(`Cannot create single-file bundle: native modules detected (${deps.nativeModules.join(', ')}). ` +
 ...
 */
function getEsbuildFormat(format) {
    switch (format) {
        case 'esm':
            return 'esm';
        case 'cjs':
 ...
 */
async function bundleWatch(options, esbuildConfig) {
    const startTime = Date.now();
 ...
 */
async function getOutputSize(outputPath) {
    try {
 ...
 */
async function getDirectorySize(filePaths) {
    let totalSize = 0;
 ...
 */
export async function stopWatch(context) {
