...
npx tsx src/servers/sseServer.ts src/config-test-sse.json > /tmp/mcp-sse-server.log 2>&1 &
SERVER_PID=$!
...
if [ $WAIT_COUNT -eq $MAX_WAIT ]; then
  ...
  pkill -f "tsx src/servers/sseServer.ts src/config-test-sse.json" 2>/dev/null || true
  exit 1
fi
echo ""
