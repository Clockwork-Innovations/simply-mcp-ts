...
test_watch_startup() {
  timeout 5s node dist/src/cli/index.js run test-watch-server.ts --watch --verbose > /tmp/watch-test.log 2>&1 &
  WATCH_PID=$!
  sleep 2
...
test_file_change_restart() {
  timeout 10s node dist/src/cli/index.js run test-watch-server.ts --watch --verbose > /tmp/watch-restart.log 2>&1 &
  WATCH_PID=$!
  sleep 2
...
test_stdio_transport() {
  timeout 3s node dist/src/cli/index.js run examples/class-minimal.ts --watch > /tmp/watch-stdio.log 2>&1 &
  STDIO_PID=$!
  sleep 2
...
test_http_transport() {
  timeout 3s node dist/src/cli/index.js run examples/class-minimal.ts --watch --http --port 3000 > /tmp/watch-http.log 2>&1 &
  HTTP_PID=$!
  sleep 2
...
test_functional_api() {
  timeout 3s node dist/src/cli/index.js run examples/single-file-basic.ts --watch > /tmp/watch-func.log 2>&1 &
  FUNC_PID=$!
  sleep 2

  if ps -p $FUNC_PID > /dev/null;
...
# Test 6: Polling mode
test_polling_mode() {
  timeout 3s node dist/src/cli/index.js run examples/class-minimal.ts --watch --watch-poll --watch-interval 200 > /tmp/watch-poll.log 2>&1 &
  POLL_PID=$!
  sleep 2
...
test_graceful_shutdown() {
  node dist/src/cli/index.js run test-watch-server.ts --watch --verbose > /tmp/watch-shutdown.log 2>&1 &
  SHUTDOWN_PID=$!
  sleep 2
...
test_dependency_watching() {
  timeout 5s node dist/src/cli/index.js run test-watch-server.ts --watch --verbose > /tmp/watch-deps.log 2>&1 &
  DEPS_PID=$!
  sleep 2
...
test_restart_timing() {
  timeout 10s node dist/src/cli/index.js run test-watch-server.ts --watch --verbose > /tmp/watch-timing.log 2>&1 &
  TIMING_PID=$!
  sleep 2
